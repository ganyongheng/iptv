...
@Component
public class MobileOrderSingleFailReSendTask extends QuartzJobBean {

    private static final Logger logger = LoggerFactory.getLogger("mobileOrderSingleFailReSendTask");
    private static final Logger log = LoggerFactory.getLogger(MobileOrderSingleFailReSendTask.class);

    private static int MAX_WORKER_NUM = Runtime.getRuntime().availableProcessors() * 2 + 1;

    static {
        MobileOrderSingleFailReSendTask.MAX_WORKER_NUM = 5;
        log.info("订单请求腾讯下单失败重试的线程数：" + MobileOrderSingleFailReSendTask.MAX_WORKER_NUM);
    }

    private static ExecutorService pool = Executors
            .newFixedThreadPool(MobileOrderSingleFailReSendTask.MAX_WORKER_NUM);
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        log.debug("【mobileOrderSingleFailReSendTask】开始执行任务..");
        ...
        try {
            ...
            log.error("【mobileOrderSingleFailReSendTask】获取本机服务ip端口出错："+e.getCause(),e);
        }
        //执行任务时，上锁（分布式锁）
        try {
            //成功拿到锁之后 ，给锁加个超时时间，防止突然中断服务，导致锁不能释放，默认1个小时失效
            boolean lock=redisCache.setnxWithExptime("MobileOrderSingleFailReSendTask_Lock", localIp,60*60);
            if(lock){
                try {
//					redisCache.putCacheWithExpireTime("MobileOrderFailReSendTask_Lock", localIp, 60*60);
                    ...
                    //到redis集合取数据
                    Set<String> dataSet=redisCache.zRevRangeByScore("MobileOrderSingleResendDequeueTask_SortSet", score, nums, String.class);

                    if(!dataSet.isEmpty()&&dataSet.size()>0){
                        //如果能取出集合数据，根据set的数据到指定的数据里面取出待出列的数据并push到重发队列中
                        for(String key:dataSet){
                            String data=redisCache.getCache("MobileResendSingleOrderData_"+key, String.class);
                            if(StringUtils.isBlank(data)){
                                logger.info("单包月订单重发任务出发送队列时，根据key[MobileResendSingleOrderData_"+key+"]获取不到数据");
                                continue;
                            }
                            JSONObject dataJson = JSONObject.parseObject(data);
                            boolean userLock = redisCache.setnxWithExptime("MobileOrderSingleLock_" + dataJson.getString("source") +"_"+ dataJson.getString("userId"), dataJson.getString("extTraceno"),30);
                            if(userLock){
                                // 线程池执行具体方法
                                pool.execute(new MobileOrderFailResendForTask(data));
                                //移除集合里面的数据
                                redisCache.zRem("MobileOrderSingleResendDequeueTask_SortSet", key);
                                redisCache.del("MobileResendSingleOrderData_"+key);
                            }
                            continue;
                        }
                    }else{
//						logger.info("当前任务BiliOrderReConfirm_Set需要发送数据为空");
                    ...
                    try {
                        //解锁
                        redisCache.del("MobileOrderSingleFailReSendTask_Lock");
                    } catch (Exception e) {
    ...
    private class MobileOrderFailResendForTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                //移动失败订单重试
                mobileOrderInterService.createAndConfirmOrderSingleResend(JSONObject.parseObject(jsonData));
            } catch (Exception e) {
                logger.error("失败订单数据"+jsonData+"重试出错："+e.getCause(),e);
            }finally{
                JSONObject dataJson = JSONObject.parseObject(jsonData);
                redisCache.del("MobileOrderLock_"+ dataJson.getString("source") +"_"+ dataJson.getString("userId"));
            }
        }
    }
